...
--The program should be run by executing the Main.exe file from the command line with three paramters.
--The first one is the grammar input file, the second one is the words input file, and the last one is the output file.
...
--The output should be displayed on the console as well. It will display the words read from the file, the converted grammar and a list of
--boolean values indicating if the word at the corresponding position is in the language or not.

--how to run: cmd>Main grammar.txt words.txt out.txt
